...
duration (1-10 seconds), then automatically reconnects.
...
class LagswitchApp:
    def __init__(self, root):
        ...
        self.armed = False
        self.bound_key = None                  # pynput key object
        self.bound_key_label = "Bind"          # human-readable
        self.duration = tk.IntVar(value=3)
        self.duration_display = tk.StringVar(value="3 seconds")
    ...
    def show_config(self):
        ...
        self._connector_row(outer, "DISCONNECT FOR", self._build_duration_menu)
    ...
    def _build_duration_menu(self, parent):
        menu_btn = tk.Menubutton(
            parent,
            textvariable=self.duration_display,
            font=self.body_font,
            bg=BADGE_BG,
            fg=BADGE_FG,
        ...
        for sec in range(1, 11):
            label = f"{sec} second" + ("" if sec == 1 else "s")
            dropdown.add_radiobutton(
                label=label,
                value=sec,
                variable=self.duration,
                command=lambda s=sec, lbl=label: self.duration_display.set(lbl),
            )
